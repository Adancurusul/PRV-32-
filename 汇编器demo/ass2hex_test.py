...
class change_into_bin(split_str):  # 直接将每行完成转化
    ...
    def intToBin12(self, i):
        return (bin(((1 << 12) - 1) & i)[2:]).zfill(12)

    def change16_x(self, wei, str1):  # 将16进制数转化为二进制
        if (str1[0] == '-'):
            head = 1
            str1 = str1[1:]
            ...
            q = str(int(str1, 16))
            q0 = int('-' + q)
            ret = self.intToBin12(q0)
        else:

            head = 0
            q = int(str1, 16)
            ret = self.intToBin12(q)
        ...
        if head:
            ...
            ret = '1' + str(bin(q))[3:].zfill(wei)
            ret = ret[0:12]
        else:
            ret = '0' + str(bin(q))[2:].zfill(wei)
            ret = ret[0:12]
    ...
    def do_with_fucking_csr(self, str11):  # fucking csr!!!!fuck!!!!!!
        ...
        return csr_head[self.str_of_csr]()
    ...
    def ADDI(self):
        ...
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '000' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def SLTI(self):
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '010' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def SLTIU(self):
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '011' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def ANDI(self):
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '111' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def ORI(self):
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '110' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def XORI(self):
        st = self.change16(12, self.list[3]) + self.change(5, self.list[2]) + '100' + self.change(5, self.list[
            1]) + '0010011'
        return st

    def SLLI(self):
        st = '0000000' + self.change16(5, self.list[-1]) + self.change(5, self.list[2]) + '001' + self.change(5,
                                                                                                              self.list[
                                                                                                                  1]) + '0010011'
        return st

    def SRLI(self):
        st = '0000000' + self.change16(5, self.list[-1]) + self.change(5, self.list[2]) + '101' + self.change(5,
                                                                                                              self.list[
                                                                                                                  1]) + '0010011'
        return st

    def SRAI(self):
        st = '0100000' + self.change16(5, self.list[-1]) + self.change(5, self.list[2]) + '101' + self.change(5,
                                                                                                              self.list[
                                                                                                                  1]) + '0010011'
        return st

    def ADD(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '000' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SUB(self):
        st = '0100000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '000' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SLL(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '001' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SLT(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '010' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SLTU(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '011' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def XOR(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '100' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SRL(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '101' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def SRA(self):
        st = '0100000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '101' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def OR(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '110' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st

    def AND(self):
        st = '0000000' + self.change(5, self.list[-1]) + self.change(5, self.list[2]) + '111' + self.change(5,
                                                                                                            self.list[
                                                                                                                1]) + '0110011'
        return st
    ...
    def JALR(self):
        st = self.change16(12, self.list[-1]) + self.change(5, self.list[2]) + '000' + self.change(5, self.list[
            1]) + '1100111'
        return st

    def BEQ(self):
        s = self.change16_x(12, self.list[-1])
        st1 = s[0:7]
        st2 = s[7:12]
    ...
    def CSRRW(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change(5, self.list[-1]) + '001' + self.change(5, self.list[
            1]) + '1110011'
        return st

    def CSRRS(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change(5, self.list[-1]) + '010' + self.change(5, self.list[
            1]) + '1110011'
        return st

    def CSRRC(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change(5, self.list[-1]) + '011' + self.change(5, self.list[
            1]) + '1110011'
        return st

    def CSRRWI(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change16(5, self.list[-1]) + '101' + self.change(5,
                                                                                                             self.list[
                                                                                                                 1]) + '1110011'
        return st

    def CSRRCI(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change(5, self.list[-1]) + '111' + self.change(5, self.list[
            1]) + '1110011'
        return st

    def CSRRSI(self):
        st = self.do_with_fucking_csr(self.list[-2]) + self.change(5, self.list[-1]) + '110' + self.change(5, self.list[
            1]) + '1110011'
        return st
...
class do_scan():  # 扫描类
    ...
    line_of_file = 0
    label_scan = 0
    parttern_label = r'(\w+\s*) [:] (.+)'  # 检测是不是label
    ...
    def label_store(self):  # 将label 移入字典
        ...
        if (not self.labelname and self.check):  # 无label定义前的代码处理
            print('c')
            self.no_label = 0
    ...
    def give_location(self):  # 给每个data以及函数分配地址
        ...
        if self.size_of_label:
            for name in self.size_of_label.keys():
                ...
                self.start_position = int(self.increase) + int(self.start_position)  # 给开始地址加入相应地址
                self.start_position = hex(self.start_position)
        ...
        if self.size_of_data:
            ...
            for name in self.size_of_data.keys():
                ...
                print('start_position', type(self.data_start_position))
        ...
        if self.label_data_dict:
            ...
            for name in self.label_data_dict.keys():  # 获取label
                for i in range(len(self.label_data_dict[name])):  # 得到label下的每行
                    ...
                    self.changed = change_into_bin(self.label_data_dict[name])  # 利用转化类进行转化
                    self.chan = bin2hex(self.changed.after_change_str).zfill(4)
                    self.high = self.chan[0:2]
                    self.low = self.chan[2:]  # 高低位互换
                    self.word = self.low + self.high  # 互换后的16进制数
                    self.location = self.label_location_dict[name]  # 得到函数初始地址
                    print('地址', self.location)
                    self.location += dec2hex(str(hex(i) * 4))  # 换算出当前地址值
                    print('location', self.location)
                    self.load = self.location_dict.setdefault(self.location, self.word)  # 存放进地址-数据字典
        elif self.data_location_dict:
        ...
        with open(self.save_location, "w+") as sl:  # 打开待生成的hex
            ...
            for name in self.location_dict.keys():
                ...
                if len(self.location_dict[name]) == 4:
                    self.str_without_check_to_write = '02' + str(name).zfill(4) + '00' + self.location_dict[
                        name]  # 生成无校验的str
                else:
                    self.str_without_check_to_write = '04' + str(name).zfill(4) + '00' + self.location_dict[
                        name]  # 生成无校验的str
